...
class Project_tender_publishing extends MY_Controller
{
    ...
    public function manage(){
        ...
        $data['tender_publishing_event']=$this->Project_tender_publishing_model->get_tender_publishing_event($data['project_id']);
       ...
       /* code start */
        $data['rap_publication']=$this->Project_tender_publishing_model->rap_publication($data['project_id']);
        $data['last_date_of_pre_bid_query']=$this->Project_tender_publishing_model->last_prebid_query($data['project_id']);
        $data['prebid_meeting']=$this->Project_tender_publishing_model->prebid_meeting($data['project_id']);
        $data['last_date_of_publishing_corrigendum']=$this->Project_tender_publishing_model->last_date_of_publishing_corrigendum($data['project_id']);
        $data['last_date_of_receipt_of_tender_document']=$this->Project_tender_publishing_model->last_date_of_receipt_of_tender_document($data['project_id']);
        $data['opening_of_general_bid']=$this->Project_tender_publishing_model->opening_of_general_bid($data['project_id']);
        $data['publication_of_qualified_bidders_of_general_bid']=$this->Project_tender_publishing_model->publication_of_qualified_bidders_of_general_bid($data['project_id']);
        $data['evaluation_of_general_bid']=$this->Project_tender_publishing_model->evaluation_of_general_bid($data['project_id']);
        $data['evaluation_of_technical_bid']=$this->Project_tender_publishing_model->evaluation_of_technical_bid($data['project_id']);
        $data['presentation_schedule']=$this->Project_tender_publishing_model->presentation_schedule($data['project_id']);
        $data['publication_of_technically_qualified_bidders']=$this->Project_tender_publishing_model->publication_of_technically_qualified_bidders($data['project_id']);
        $data['opening_of_financials']=$this->Project_tender_publishing_model->opening_of_financials($data['project_id']);
        $data['evaluation_of_financial_bids']=$this->Project_tender_publishing_model->evaluation_of_financial_bids($data['project_id']);
        $data['declaration_of_results']=$this->Project_tender_publishing_model->declaration_of_results($data['project_id']);
      /* code end */
        ...
        if ($this->form_validation->run() == FALSE) {
          ...
          // start
		   $db_data['project_id'] = base64_decode($_REQUEST['project_id']);
                ...
                $db_data['prime_value'] = $this->checkEmptyValue($_REQUEST['primeval']);
                $db_data['auto_value'] = $this->checkEmptyValue($_REQUEST['autoval']);
         ...
         /* new datas */
              ...
              if (!empty($data['tender_publishing_event'])) {
                $del_location_data =$this->Project_tender_publishing_model->remove_tender_publishing_event_data($project_id);
              
              //$del_location_data =$this->Project_model->removeamount($project_id);
                  
            }
            $amount_count = count($_REQUEST['event_date']);
              for ($i=0; $i < $amount_count; $i++) { 
                    
                        $event_date =!empty($_REQUEST['event_date'][$i])?$_REQUEST['event_date'][$i]:'NULL';
                     //  $project_amount_id=$this->input->post('_id')[$i];
                          $insert_data = array(
                          'project_id' => $project_id, 
                          'event_type' => $this->checkEmptyValue($_REQUEST['event_type'])[$i],
                          'event_date' =>$event_date,
                          'event_time' => $this->checkEmptyValue($_REQUEST['event_time'])[$i],
                          'event_venue' => $this->checkEmptyValue($_REQUEST['event_venue'])[$i],
                         
                      );
            
            
                      $this->Project_tender_publishing_model->addProject_tender_publishing_event($insert_data);
            }
