...
class Issue_of_PO extends MY_Controller
{   
    public function __construct()
    {
        ...
        $this->load->model('Issue_of_PO_model');
    ...
    { 
        ...
        $projectData_exist_flag = $this->Issue_of_PO_model->checkProjectExits($project_id);
        $data['get_issueofloa'] = $this->Issue_of_PO_model->getissueofloa($project_id);

        $data['fetch_issueofloa'] = $this->Issue_of_PO_model->fetchissueofloa($project_id);

        $data['fetch_negoofloa'] = $this->Issue_of_PO_model->fetchnegoofloa($project_id);
       ...
       // $this->form_validation->set_rules('negotiation_bid_value', 'Negotiated Bid Value', 'required|greater_than[0]'); 
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->load->common_template('tendering/issue_of_po', $data);
            ...
            if($submit == 'Submit'){
                ...
                if(!empty($projectData_exist_flag)){  //UPDATE
                    ...
                    $EE_last_data_update =$this->Issue_of_PO_model->updateissueofloa($db_data,$issueofloa_id);

               }

                else { //ADD

                    $EE_last_data =$this->Issue_of_PO_model->saveissueofloa($db_data);
                }

            }

            $this->session->set_flashdata('success', 'Issue of PO saved successfully');

                  if($project_id){
                       redirect('Issue_of_PO/manage?project_id=' . base64_encode($project_id));
                    }
            }
