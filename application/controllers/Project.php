...
class Project extends MY_Controller
{
    ...
    public function project_conceptualisation(){
        ...
        $data['role_id'] = $this->session->userdata('user_role_id');
        ...
        $data['amount_value']= $this->Project_model->get_amount_array($project_id);
       ...
       // }
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $db_data['project_office'] = $this->checkEmptyValue($_REQUEST['project_office']);
             $db_data['project_dept_cost'] = $this->checkEmptyValue($_REQUEST['project_dept_cost']);
...
if (!empty($data['amount_value'])) {
    $del_location_data =$this->Project_model->removeamount($project_id);
  
  //$del_location_data =$this->Project_model->removeamount($project_id);
      
}
$amount_count = count($_REQUEST['amount']);
  for ($i=0; $i < $amount_count; $i++) { 
        
   
           $project_amount_id=$this->input->post('amount_id')[$i];
              $insert_data = array(
              'project_id' => $project_id, 
              'amount' => $this->input->post('amount')[$i],
              'payment_date' =>$this->input->post('payment_date')[$i],
              'transactionid' => $this->input->post('transactionid')[$i],
              
          );


          $this->Project_model->addProjectamount($insert_data);
}
