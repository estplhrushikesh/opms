...
class Project_list extends MY_Controller
{
    ...
    function pip_conceptualisation(){
          ...
          $data['conceptualisation_data'] = $this->Project_list_model->get_project_conceptualisation_data($user_id);

          $data['project_creation_data'] = $this->Project_list_model->get_project_creation_data($user_id);
    ...
    function pip_dpr(){
          ...
          $data['project_pending_data'] = $this->Project_list_model->get_pip_dpr_pending_projects_list_data($user_id);

          $data['pip_dpr_data'] = $this->Project_list_model->get_pip_dpr_list_data($user_id);

          $this->load->common_template('project/pip_dpr_list_view', $data);
    }


    function pip_pre_project_activities(){
          $user_id = $this->session->userdata('id');
          $user_type = $this->session->userdata('user_type');
    ...
    function pip_administrative_approval(){
          ...
          $data['project_pending_data'] = $this->Project_list_model->get_pip_administrative_pending_projects_list_data($user_id);

          $data['pip_administrative_data'] = $this->Project_list_model->get_pip_administrative_list_data($user_id);
    ...
    function pip_tender_publishing(){
        $user_id = $this->session->userdata('id');
          ...
          $data['project_pending_data'] = $this->Project_list_model->get_pip_tender_publishing_projects_list_data($user_id);

          $data['pip_preparation_data'] = $this->Project_list_model->get_pip_tender_publishing_list_data($user_id);
