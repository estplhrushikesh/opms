...
{   
    ...
    public function manage(){
        ...
        $data['prebid_query_file'] = $this->Prebid_model->prebid_query_file($project_id);
        ...
        if(!empty($_REQUEST['submit'])){ 
             ...
             if($submit == 'Submit'){
                 ...
                 $bidder_info = $this->input->post('bidder_info');
                 ...
                 // +++++++++++++++
                     ...
                     $prebid_file_count = count($_FILES['prebid_querydoc']['name']);
                     $prebid_query_file = $this->input->post('prebid_querydoc');
                     $prebid_fileupdate = $_REQUEST['prebid_querydoc_hidden'];
                 ...
                 // +++++++++++++++
                  ...
                  for ($i=0; $i < $bidder_name_count; $i++) {
                            ...
                            $filename = rand(11,999999). time().'_'.$_FILES['prebid_querydoc']['name'][$i];

                            
                              $this->load->library('upload',$config);

                              $_FILES['file']['name'] = $filename;
                              $_FILES['file']['type'] = $_FILES['prebid_querydoc']['type'][$i];
                              $_FILES['file']['tmp_name'] = $_FILES['prebid_querydoc']['tmp_name'][$i];
                     ...
                     if(!empty($projectData_exist_flag)){//UPDATE
                        ...
                        $prebid_id = $data['get_prebid'][0]['id'];
                         ...
                         if(empty($_FILES['prebid_querydoc']['name'][$i])) {

                            $filename = $_REQUEST['prebid_querydoc_hidden'][$i];
                    ...
                    //  if (!empty($data['prebid_query_file'])) {
        
                    //     $del_data_file =$this->Prebid_model->removequeryfile($project_id);
                    ...
                    //          $corrigendum_issuance_date =  $_REQUEST['query_date'][$i];
                    //           $this->load->library('upload',$config);
                    ...
                    //              if(!empty($_REQUEST['prebid_querydoc_hidden'][$i]) || ($_FILES['prebid_querydoc']['name'][$i])) {
                    ...
                    //              $this->Prebid_model->addbidderqueryfile($UaddData);
                    //             }
                    ...
                    //                 $this->Prebid_model->addbidderqueryfile($db_data);
                    //            }
...
function gettehsilSelection_data($country_id,$state_id){
  ...
  function details(){
    ...
    $data['prebid_bidder_details'] = $this->Prebid_model->prebid_bidder_details($project_id);
    
    
    $data['prebid_file'] = $this->Prebid_model->prebid_file($project_id);

    $data['fetch_meeting_date'] = $this->Prebid_model->fetch_meeting_date($project_id); 
    $this->load->common_template('tendering/pre_bid_details', $data);
