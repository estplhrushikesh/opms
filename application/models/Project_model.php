...
GROUP by period");
    ...
    function getProjectCreationDetails($project_id){
        $this->db->select('t1.id,t1.project_cost as estimate_total_cost,t1.cat_id,t1.project_office,t1.project_submission_date,t1.project_description,t1.scheme_id as project_group,t1.cat_id as project_type,t1.wing_id as wing_id,t1.division_id as division_id,t1.location as location_id,t1.project_name,t2.project_type as project_type_name,t3.name as area_name,t4.name as scheme_name,t5.name as circle_name,t6.wing_name,t7.division_name');
        $this->db->from('project_creation t1');
        $this -> db ->join('project_type_master t2', 't2.id = t1.cat_id ', 'left');
        $this -> db ->join('area_master t3', 't3.id = t1.location ', 'left');
        $this -> db ->join('group_master t4', 't4.id = t1.scheme_id ', 'left');
    ...
    function get_amount_array($project_id){
        $this->db->select('*');
        $this->db->from('project_amount');
        $this->db->where('project_id', $project_id);
    ...
    function updateProjectamount($data,$amount_id){
        $this->db->where('id',$amount_id);
        $this->db->update('project_amount', $data);
        return TRUE;
    }
    function addProjectamount($data){
        $insert = $this->db->insert('project_amount', $data);
        if ($insert) {
            $insert_id = $this->db->insert_id();
            return $insert_id > 0 ? $insert_id : false;
        } else {
    ...
    function removeamount($project_id){

        $this->db->where('project_id', $project_id);
        $this->db->delete('project_amount');
    }
