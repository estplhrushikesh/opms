...
class Issue_of_PO_model extends CI_Model {
   ...
   public function saveissueofloa($data) {
    $insert = $this->db->insert('tendering_issue_of_po', $data);
        if ($insert) {
    ...
    public function checkProjectExits( $project_id,$table_name = 'tendering_issue_of_po'){
        $this->db->select('*');
   ...
   public function getissueofloa($projectid) {
      $this->db->select('*');
      $this->db->where('tendering_issue_of_po.project_id', $projectid);
   ...
   public function updateissueofloa($data,$issueofloa_id){
      ...
      $this->db->update('tendering_issue_of_po', $data);
      return TRUE;
   }
