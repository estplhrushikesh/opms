...
class Monitoring_model extends CI_Model {
    ...
    public function get_project_activity($project_id=null, $project_work_item_id=null) {
      $this->db->select('pa.particulars, pfpm.project_work_item_id, pfpm.project_activity_id');
      $this->db->from('project_physical_planning_main as pfpm');
      $this->db->where('pfpm.project_id', $project_id);
      $this->db->where('pfpm.project_work_item_id', $project_work_item_id);
      $this->db->where('pfpm.activity_status','YES');
      $this->db->join('project_pf_activities as pa', 'pfpm.project_activity_id = pa.id', 'LEFT');
      $query = $this->db->get();
      return $query->result_array();
    }
    ...
    /* Get project physical activity plan */
    public function get_physical_act($project_id, $work_item_id, $activity_id){
    ...
    public function get_financial_act($project_id, $work_item_id, $activity_id){
      $this->db->select('project_financial_planning_main.*','project_pf_activities.particulars','project_pf_activities.id');
...
/*=============== New Changes on 30-07-2021 ================== */
  ...
  public function get_project_type($activity_id){
  ...
  public function add_paid_amount($tbl,$array){
    $insert = $this->db->insert($tbl, $array);
   // $insert_id = $this->db->insert_id();
   if($insert){
    return true;
   }else{
    ...
    public function add_invoice_amount($tbl,$array){
      $insert = $this->db->insert($tbl, $array);
     // $insert_id = $this->db->insert_id();
     if($insert){
      return true;
    ...
    public function total_paid_amount($pid,$wid,$aid){

      $this->db->where('project_id',$pid);
      $this->db->where('work_item_id',$wid);
      $this->db->where('activity_id',$aid);
      $query=$this->db->get('project_paid_amount');
      return $query->result_array();
     // return $num_rows;

    }
    public function total_invoice_amount($pid,$wid,$aid){
    ...
    public function total_invoice_amount_update($pid,$wid,$aid,$iid){
    ...
    public function count_tot_amount($pid,$wid,$aid){
      
      $this->db->where('project_id',$pid);
      $this->db->where('project_work_item_id',$wid);
      $this->db->where('project_activity_id',$aid);
      $query=$this->db->get('project_financial_planning_main');
      return $query->result_array();
     // return $num_rows;
    }
public function get_bidder_data($pid){
  
  $this->db->where('project_id',$pid);
 
  $query=$this->db->get('tendering_issue_of_po');
  return $query->result_array();
}
    public function get_invoice_list($pid,$wid,$aid){
      $this->db->where('project_id',$pid);
      $this->db->where('work_item_id',$wid);
      $this->db->where('activity_id',$aid);
      $query=$this->db->get('project_invoice');
      ...
      public function get_invoice_row($invoice_no){
     
      $this->db->where('invoice_no',$invoice_no);
      $query=$this->db->get('project_invoice');
      return $query->result_array();
      }
      public function get_invoice_row_id($invoice_no){
     
        $this->db->where('id',$invoice_no);
        $query=$this->db->get('project_invoice');
        return $query->result_array();
        }
      public function get_invoice_paid_details($invoice_no){
        $this->db->where('invoice_id',$invoice_no);
        $query=$this->db->get('project_paid_amount');
        return $query->result_array();

      }
      public function count_paid_rows($invoice_no){
        $this->db->where('invoice_id',$invoice_no);
    ...
    public function update_invoice_amount($table,$array,$iid){
      $this->db->where('id',$iid);
      $this->db->update($table, $array);
      return TRUE;
    }
    public function count_invoice_no($invoice_no){
    ...
    public function count_invoice_no_update($iid,$invoice_no){
      $this->db->like('invoice_no',$invoice_no);
      $this->db->where('id!=',$iid);
     $query=$this->db->get('project_invoice');
     $num_rows = $query->num_rows();
     return $num_rows; 
      ...
      /* $query=$this->db->select('project_paid_amount.*');
      $query=$this->db->where('project_id',$pid);
     $query= $this->db->where('work_item_id',$wid);
      $query=$this->db->where('activity_id',$aid);
...
/*   public function get_project_physical_activity_details($project_id,$work_item_id){
    $this->db->select('*');
    $this->db->join('project_pf_activities ', 'project_physical_planning_main.deliverable_name = project_pf_activities.id', 'LEFT');
    $this->db->where('project_physical_planning_main.project_id', $project_id);
