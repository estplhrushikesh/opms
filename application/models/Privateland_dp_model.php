...
class Privateland_dp_model extends CI_Model {
  ...
  public function save_priland_dp($data) {
      $query =  $this->db->insert('pre_project_activity_mis', $data);
          if ($insert) {
  ...
  public function update_priland_dp($data,$dp_id) {
      ...
      $this->db->update('pre_project_activity_mis', $data);
      return TRUE;
  }
function fetch_district()
 {
  ...
  return $query->result();
        ...
        public function checkProjectExits( $project_id,$table_name = 'pre_project_activity_mis'){
        $this->db->select('*');
   ...
   public function getprivatelanddp($projectid) {
      $this->db->select('*');
      $this->db->where('pre_project_activity_mis.project_id', $projectid);
      $this->db->limit('1');
      $query = $this->db->get('pre_project_activity_mis');
      return $query->result_array();
   }
