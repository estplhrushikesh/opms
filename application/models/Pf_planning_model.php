...
class Pf_planning_model extends CI_Model {
 ...
 /*delete for all*/ 
   ...
   /*Get project activity list*/
    ...
    public function fetch_deliverables($project_id,$project_work_item_id=Null) {




      $this->db->select('project_pf_activities.*');
      $this->db->join('project_pf_activities','project_physical_planning_main.project_activity_id = project_pf_activities.id','left');
      $this->db->join('project_financial_planning_main','project_financial_planning_main.deliverable_name = project_pf_activities.id','left');
      $this->db->where('project_physical_planning_main.project_id', $project_id);
      $this->db->where('project_pf_activities.id NOT IN (select deliverable_name from project_financial_planning_main)');
     
      if(!empty($project_work_item_id)){



       $this->db->where('project_physical_planning_main.project_work_item_id', $project_work_item_id);
      }
   ...
   public function project_planned_value($pid){
    $this->db->where('project_id',$pid);
    $query = $this->db->get('project_financial_planning_main');
    return $query->result_array();
   }
   public function activity_all_or_not($pid,$wid){
      $this->db->where('project_id',$pid);
      $this->db->where('project_work_item_id',$wid);
      $this->db->where('project_activity_id',0);
      $query = $this->db->get('project_financial_planning_main');
      return $query->num_rows();
   }
   public function activity_all_or_not_physical($pid,$wid){
    $this->db->where('project_id',$pid);
    $this->db->where('project_work_item_id',$wid);
    $this->db->where('project_activity_id',0);
    $query = $this->db->get('project_physical_planning_main');
    return $query->num_rows();
      ...
      public function activity_added_or_not($pid,$wid){
        $this->db->where('project_id',$pid);
        $this->db->where('project_work_item_id',$wid);
   ...
   public function get_project_financial_activity_details($project_id,$project_work_item_id) {
      $this->db->select('*');
      $this->db->join('project_pf_activities ', 'project_financial_planning_main.deliverable_name = project_pf_activities.id', 'LEFT');
      $this->db->where('project_financial_planning_main.project_id', $project_id);

      $this->db->where('project_financial_planning_main.project_work_item_id', $project_work_item_id);
...
/* checking activity id exist or not */
    ...
    public function update_state_status($table,$project_id,$activity_id){
      $this->db->where(['project_id' => $project_id,'id'=> $activity_id]);
      $this->db->update($table, ['stage_status'=>'y']);
      //echo $this->db->last_query().'<br><br>';
      return TRUE;
    }
    public function get_added_activity($project_id,$item_id){
      $this->db->select('*');
      $this->db->join('project_pf_activities','project_physical_planning_main.project_activity_id = project_pf_activities.id','left');
      $this->db->where('project_physical_planning_main.project_id', $project_id);
      ...
      if(!empty($item_id)){
        ...
        $this->db->where('project_physical_planning_main.project_work_item_id', $item_id);
      }
      $this->db->where('project_physical_planning_main.project_activity_id!=',0);
      $query = $this->db->get('project_physical_planning_main');
      return $query->result_array();
    }
    public function get_new_activity($project_id){
       $sql="Select * from project_pf_activities where project_id=".$project_id." and stage_status = 'N'";    
      $query = $this->db->query($sql);
      return $query->result_array(); 


     /*  $array = array('stage_status !=' => 'y', 'project_id' => $project_id);
      $this->db->select('project_pf_activities.*');
      
      $this->db->where('project_pf_activities.project_id', $project_id);
      $this->db->where('project_pf_activities.stage_status!=', '2');
      $this->db->where($array); */
     
    //  $query = $this->db->get('project_pf_activities');
    //  return $query->result_array();
    }
    function update_physical_activity_status($tableName,$activity_id,$data){
 ...
 /*delete for all*/ 
   ...
   function delete_physical_added_activity($tableName,$project_id,$work_item_id,$activity_id){
   ...
   function get_financial_main_da($pid,$wid){
   // echo $pid;
   // echo $wid;
    //echo $aid;
    $this->db->select('project_financial_planning_main.*');
    $this->db->where('project_financial_planning_main.project_id', $pid);
  //  $this->db->where('project_financial_planning_main.project_work_item_id', $wid);
   // $this->db->where('project_financial_planning_main.project_activity_id', $aid);
   // $this->db->where('project_financial_planning_main.status', 'Y');
    $query = $this->db->get('project_financial_planning_main');
    return $query->result_array();
   }
