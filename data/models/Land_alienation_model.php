...
class Land_alienation_model extends CI_Model {
  ...
  public function savealienation($data) {
        ...
        $insert = $this->db->insert('pre_project_activity_erp', $data);
        if ($insert) {
  ...
  public function updatealianation($data,$landalination_id) {
      ...
      $this->db->update('pre_project_activity_erp', $data);
      return TRUE;
  }
...
function fetch_district()
 {
  ...
  return $query->result();
        ...
        public function checkProjectExits( $project_id,$table_name = 'pre_project_activity_erp'){
        $this->db->select('*');
  ...
  public function getlandalianation($projectid) {
      $this->db->select('*');
      $this->db->where('pre_project_activity_erp.project_id', $projectid);
      $this->db->limit('1');
      $query = $this->db->get('pre_project_activity_erp');
      return $query->result_array();
   }
