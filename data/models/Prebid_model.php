  ...
  function __construct(){
   ...
   public function addbidderdata($data){
    ...
    public function addbidderdatafile($data){
     ...
     public function addbidderqueryfile($data){

        $insert = $this->db->insert('tendering_pre_bid_query_document', $data);
        if ($insert) {
            $insert_id = $this->db->insert_id();
            return $insert_id > 0 ? $insert_id : false;
        } else {
    ...
    public function removequeryfile($project_id) {
      $this->db->where('project_id', $project_id);
      $this->db->delete('tendering_pre_bid_query_document');
    }
    ...
    public function prebid_bidder_details($project_id){
      $this->db->select('*');
      $this->db->where('tendering_pre_bid_bidder_details.project_id', $project_id);
      $query = $this->db->get('tendering_pre_bid_bidder_details');
      return $query->result_array();
    ...
    public function prebid_query_file($project_id){
      $this->db->select('*');
      $this->db->where('tendering_pre_bid_query_document.project_id', $project_id);
      $query = $this->db->get('tendering_pre_bid_query_document');
      return $query->result_array();
    }
