...
class Pf_planning extends MY_Controller
{
   ...
   public function get_earned_financial_deliverable_data(){
        $project_id= $_REQUEST['project_id'];
        $project_work_item_id= $_REQUEST['project_work_item_id'];
        $activity_id= $_REQUEST['activity_id'];
        
        $financial_deliverable_details=$this->Pf_planning_model->get_financial_act($project_id, $project_work_item_id, $activity_id);
        ...
        if(!empty($_REQUEST['submit'])){
           ...
           $activitiesremaining_amt = $get_activities_budget_amt - $get_activities_planned_amt;
            ...
            if(empty($_REQUEST['project_financial_id'])){
               ...
               // redirect('pf_planning/add_financial_planning?project_id='.base64_encode($_REQUEST['project_id']).'&project_work_item_id='.base64_encode($_REQUEST['project_work_item_id']));
           ...
           // $total_financial_planning=0;
            ...
            // $where = array('project_id' => $_REQUEST['project_id'],'project_work_item_id' => $_REQUEST['project_work_item_id'],'project_activity_id' => $_REQUEST['activity_id']);
            // $this->Pf_planning_model->updateDataCondition('project_financial_planning_main', $financial_planning, $where);
