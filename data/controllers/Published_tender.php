...
class Published_tender extends MY_Controller
{
    ...
    /* end for PRE-BID listing Page */
     ...
     /* for pre-qualification Evalution  listing Page */

    function pp_pre_qualification(){
          $user_id = $this->session->userdata('id');
          $user_type = $this->session->userdata('user_type');
        
          $data['published_tender_data'] = $this->Published_tender_model->pre_qualification_project_lists($user_id);

           $this->load->common_template('published_tender/pre_qualification_project_list', $data);
    }

    /* end for pre-qualification  listing Page */
    ...
    function pp_tender(){
          ...
          $data['published_tender_data'] = $this->Published_tender_model->negotiation_project_lists($user_id);
           ...
           $this->load->common_template('published_tender/tender_project_list', $data);
    }
    ...
    function pp_issue_loi(){
          $user_id = $this->session->userdata('id');
          $user_type = $this->session->userdata('user_type');
        
          $data['published_tender_data'] = $this->Published_tender_model->issue_loa_project_lists($user_id);

           $this->load->common_template('published_tender/issue_loi_project_list', $data);
    }


    /* end for Issue of LoA listing Page */

     function pp_issue_po(){
          $user_id = $this->session->userdata('id');
          $user_type = $this->session->userdata('user_type');
        
          $data['published_tender_data'] = $this->Published_tender_model->issue_loa_project_lists($user_id);

           $this->load->common_template('published_tender/issue_po_project_list', $data);
    }
